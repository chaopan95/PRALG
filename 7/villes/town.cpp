...
bool Town::operator < (const Town &T) const//define < for class Town
{
    //compare the names of two towns, if they are identic, compare their coordinates
    return (_name < T.name()) || ((_name == T.name()) && (_x < T.x() || (_x == T.x() && _y < T.y())));
}
...
//
inline float degreeToRadian(float deg){ return (deg/180)*float(M_PI); }
const float a = 6378137;       // semi-major axis of the ellipsoid
const float e = float(0.08181919106); // first eccentricity of the ellipsoid
