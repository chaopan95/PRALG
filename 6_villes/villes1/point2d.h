...
public:
	...
	Point2D(float x, float y, T info) : Point_2D(x, y), _info(info) { _infos.push_back(info); }
	// Get information
    // It can be used both to get or set the value: p.info() = newValue;
	inline void addInfo(T info) { _infos.push_back(info); }
	inline T& info() { return _info; }
	inline vector<T>& infos() { return _infos; }
};
